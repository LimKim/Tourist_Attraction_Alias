...
class LanguageModel(object):
    ...
    def _build_word_embeddings(self):
        ...
        with tf.device("/cpu:0"):
            ##################################### 这段自己加
            embedding_addr = './corpus_dir/vectors.txt'
            import gensim
            w2v_model = gensim.models.KeyedVectors.load_word2vec_format(fname=embedding_addr)
            tmp_embed = np.zeros((n_tokens_vocab, projection_dim), dtype=DTYPE)
            words = w2v_model.vocab
            for i, word in enumerate(words):
                tmp_embed[i+3][:] = w2v_model[word]  # +3 是因为前三个是 <S>  </S>  <UNK>
...
def train(options, data, n_gpus, tf_save_dir, tf_log_dir,
    ...
    with tf.Session(config=tf.ConfigProto(
        ...
        for batch_no, batch in enumerate(data_gen, start=1):
            ...
            if batch_no % 1250 == 0:
                summary_writer.add_s:ummary(ret[3], batch_no)
            if batch_no % 100 == 0:
            ...
            if (batch_no % 1250 == 0) or (batch_no == n_batches_total):
                ...
                with h5py.File('./laiye_vocab_embedding.hdf5', 'w') as fout:
